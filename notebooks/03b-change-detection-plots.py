...
def plot_chunk_data(chunk_df, start_dt, end_dt, plot_dir):
    ...
    cbar.ax.text(
        1.5, 0.01, start_dt.strftime("%Y-%m-%d %H:%M:%S"), transform=cbar.ax.transAxes, ha="left", va="bottom", rotation=90
    )
    # Position the end date (top of colorbar)
    cbar.ax.text(
        1.5, 0.99, end_dt.strftime("%Y-%m-%d %H:%M:%S"), transform=cbar.ax.transAxes, ha="left", va="top", rotation=90
    )
